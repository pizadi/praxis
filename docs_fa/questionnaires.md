...
- **هم‌ترازی با یک کورپوس مشترک تضمین می‌شود**:
  `testdata/questionnaire_parity.json` (۱۰۷ مورد: نتیجهٔ ارزیابی، اعتبار
  فرمول، رشته‌های خطای پاسخ، ماتریس قالب) هم با pytest
  (`backend/tests/parity/`) و هم با vitest (`frontend/src/parity/`) اجرا
  می‌شود. این کورپوس مرزهای طول/عمق (`MAX_FORMULA_LENGTH = 1000` و
  `MAX_DEPTH = 100` — هر دو طرف)، قرارداد پیام‌های خطا با `faAnswerError`
  و قاعدهٔ «فرمولِ فقط-فاصله = بدون نمره» (مقدار هنگام اعتبارسنجی trim
  می‌شود) را قفل می‌کند. ولیدیتورها را فقط همراه کورپوس و هر دو رانر تغییر
  دهید.
