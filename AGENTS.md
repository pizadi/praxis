...
- **Questionnaires** (branch `feature/questionnaires`): templates (`questionnaire_templates.format_json`) are validated JSON documents — pydantic `QuestionnaireFormat` in `app/services/questionnaires.py` (discriminated union: `number` with min/max/integer/unit, `choice` with 2–50 options each optional `score`, `string` with multiline/max_length; keys `^[a-z0-9_]{1,64}$` unique per template; `extra="forbid"`). Every write path (API create/update, builder, **JSON upload**) funnels through the same validation; `POST /questionnaires/templates/validate` checks without saving. Responses belong to a **patient** (`questionnaire_responses.patient_id`, NOT appointment), multiple per template allowed, `answers_json` is raw nullable JSON. Server-side answer validation rejects unknown keys and type/range/choice violations only — **null/absent is always valid** (`required` is UI-level). **No snapshot**: render merges stored answers with the *current* template (`frontend/src/lib/questionnaire.ts::mergeResponse`) — removed keys ignored, new questions null, invalid values "missing" + warning banner + confirm-dialog clear (PATCH re-validated). Access: `questionnaires.read`/`fill` (receptionist+ by default), `questionnaires.templates` (admin) for template CRUD.
